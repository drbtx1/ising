# -*- coding: utf-8 -*-
"""
Created on Fri May 16 13:16:30 2025
...
for deltaJ in np.arange(-2,2,0.01):
    deltaJList.append(deltaJ)
    imthreshg = np.nan
    for string in strings[3:4]:   
    #print(string)
        nonHermitianTerms = eval(string)
    thresholdFound = False
    g = 0
    while thresholdFound == False and g < 1.0:
        ...
        for e in ev:
            ...
            if im(e) > 10e-4:
                ...
                print(imthreshg)
        g += 0.001     
    thresholdList.append(imthreshg)    
print(thresholdList)
plt.xlabel(r'$\Delta$J/J')
plt.ylabel('$\gamma_{PT}/J$')
plt.plot(deltaJList, thresholdList)
end = time.time()
print(end-start)