...
realPart = []
imagPart = []
for g in np.arange(0,0.75, 0.005):
...
ax.set_ylabel("Im(E/J)", rotation = 0)
#ax.yaxis.set_label_coords(1.1, 1.1)
...
for i in range(0,2**N):
    ...
    ax.scatter3D(gamma_array[i,:], imagPart_array[i,:], realPart_array[i,:], marker = ".", c = colors, s = 1)
ax.view_init(elev = 5, azim = -80)   #defaults are 30, -60 
plt.tight_layout()
plt.show(-1,0)
