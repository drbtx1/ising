# -*- coding: utf-8 -*-
"""
Created on Sat Feb 22 19:57:17 2025
...
pq_pairs = [(0,6), (0,5), (1,5)]
#hList = [0.0]
markerdict = {(0,6):"^", (0,5):"o", (1,5):2}
colordict = {(0,6):"blue", (0,5):"orange", (1,5):"brown"}
sitedict = {(0,6):"Edge", (0,5):"Edge + Bulk", (1,5):"Bulk"}
labeldict = {(0,6):r'$\Gamma^{"+"}_{"0,6"}$', 
             (0,5):r'$\Gamma^{"+"}_{"0,5"}$', 
             (1,5):r'$\Gamma^{"+"}_{"1,5"}$'}

def find_threshold(pair):
    p = pair[0]
    q = pair[1]
    
    
    g_threshold = np.zeros(len(JList))
    for idx in range(len(JList)):
        J = JList[idx]
        found_threshold = False
        for idxg in range(len(gList)):
            print(idxg)
            if found_threshold == True:
                break
            g = gList[idxg]
    ...
    plt.plot(JList, g_threshold, label =  sitedict.get(pair), linewidth = 0.5,
             marker = markerdict.get(pair, '.'), ms = 3, color = colordict.get(pair, 'green'))        
            
for pair in pq_pairs:
    find_threshold(pair)                    
