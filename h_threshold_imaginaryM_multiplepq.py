# -*- coding: utf-8 -*-
"""
Created on Sun Feb 16 13:26:10 2025
...
def find_h_threshold(hList, pair):
    p = pair[0]
    q = pair[1]
    maxIm = np.zeros((len(hList),(int((maxg-ming)/dg))))
        ...
        for idxg in range(len(gArray)):
            ...
            for e in ev:
                if im(e) > 10e-10 and im(e) > maxIm[idx][idxg]:
                    maxIm[idx][idxg] = im(e)
        plt.plot(gArray, maxIm[idx], label = "hz/J = " + str(h) + " (p,q) = " + str(pair))        
            
for pair in pq_pairs:
    find_h_threshold(hList, pair)                    
                
    
    
#plt.imshow(maxIm,extent = [-1,1,-1,1], origin='lower')
plt.title("Maximum imaginary part")
plt.legend()
plt.xscale("log")
plt.xlabel("gamma/J")
plt.ylabel("Largest imaginary part of eigenvalue")
#plt.ylim(0,0.15)
#plt.xlim([min,max])
#plt.ylim([min,max])
