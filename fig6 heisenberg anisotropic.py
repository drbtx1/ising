# -*- coding: utf-8 -*-
"""
Created on Sun Feb 23 20:03:59 2025
...
min = -1 
g = []
for gamma in np.arange(0,1,dg):
    g.append(gamma)
H = []
dh = 0.01
for h in np.arange(0,0.5, dh):
    H.append(h)
maxIm = np.zeros((len(g), len(H)))
#print(maxIm)
print(int((max-min)/dg))
...
for idxg in range(len(g)):   
    ...
    for idxh in range(len(H)):   
        ...
        #nonHermitianTerms = {p: np.array([[0, gplus],[gminus, 0]])}
        ham = findPerturbedHamiltonian(N,J,deltaJ,h,gplus, periodic, 
        ...
        for e in ev:
            ...
            if im(e) > maxIm[idxg][idxh] and im(e) > 10e-6:
                maxIm[idxg][idxh] = im(e)    
            
                    
                
     
   
plt.imshow(maxIm, origin='lower',extent = [0,0.5,0,1], aspect = 'auto')
plt.title(r'$\gamma\sigma^{+}_{0}$', loc='left')
plt.xlabel(r'$h_{z}/J$')
plt.ylabel(r'$\gamma_{PT}/J$')
#plt.xlim([min,max])
#plt.ylim([min,max])
plt.colorbar()
